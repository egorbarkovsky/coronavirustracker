...
internal class TrackingServiceTest : Spek({

    describe("coronavirus service") {

        val coronavirusDataProviderMock = mock<CoronavirusDataProvider>()

        val trackingService = DefaultTrackingService(coronavirusDataProviderMock)
        ...
        on("correct coronavirus data from single country") {

            val dataFromSingleConutry =
            ...
            whenever(coronavirusDataProviderMock.getCoronavirusData())
                .thenReturn(dataFromSingleConutry)

            val result = trackingService.getCountriesStats()
